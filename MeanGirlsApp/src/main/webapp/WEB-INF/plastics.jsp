			...
			<c:when test="${user.role=='user' }">
				...
				<c:choose>
					<c:when test="${student.totalFetch < clique.minimumFetchLevel }">
						...
						<div class="container tenor-gif-embed" data-postid="12328861"
							data-share-method="host" data-width="100%"
							data-aspect-ratio="1.7777777777777777">
							<a
								href="https://tenor.com/view/back-to-school-you-cant-sit-with-us-first-day-of-school-high-school-gif-12328861">Back
								To School You Cant Sit With Us GIF</a> from <a
								href="https://tenor.com/search/backtoschool-gifs">Backtoschool
								GIFs</a>
						</div>
					...
					<c:when test="${student.totalFetch >= clique.minimumFetchLevel }">
					<c:if test="${!partOfClique }"><h3>DEBUG: SHOW IF NOT PART OF CLIQUE</h3>
					<form action="addStudentToClique.do" method="POST">
										<button>Join this clique!</button>
					...
					<form action="addPlasticsComment.do" method="POST">
						<input type="hidden" name="studentId" value="${student.id}" /> <input
							type="hidden" name="cliqueId" value="${clique.id}" /> Comment: <input
